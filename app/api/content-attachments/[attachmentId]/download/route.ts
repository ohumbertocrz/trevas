...
import { canAccessLesson, canAccessModule } from "@/application/access/lesson-access";
...
async function ownerIsPublished(ownerType: string, ownerId: string) {
  ...
  if (ownerType === "module") { const courses = await contentRepository.listCourses(); const modules = (await Promise.all(courses.map((course) => contentRepository.listModules(course.id)))).flat(); const module = modules.find((item) => item.id === ownerId); return Boolean(module?.status === "published" && courses.find((course) => course.id === module.courseId)?.status === "published"); }
  if (ownerType === "lesson") { const lesson = await contentRepository.getLesson(ownerId); if (!lesson || lesson.status !== "published") return false; const [course, modules] = await Promise.all([contentRepository.getCourse(lesson.courseId), contentRepository.listModules(lesson.courseId)]); return course?.status === "published" && modules.find((module) => module.id === lesson.moduleId)?.status === "published"; }
...
export async function GET(_request: Request, { params }: { params: Promise<{ attachmentId: string }> }) {
  ...
  if (!attachment || !attachment.storagePath.startsWith("protected/content-attachments/")) return NextResponse.json({ error: "Arquivo não encontrado." }, { status: 404 });
  if (!hasAdministrativeAccess(user.roles) && !(await ownerIsPublished(attachment.ownerType, attachment.ownerId))) return NextResponse.json({ error: "Arquivo não disponível." }, { status: 404 });
  if (!hasAdministrativeAccess(user.roles) && !hasActiveEntitlement(user.entitlement)) {
    const allowed = attachment.ownerType === "module" ? await canAccessModule(user, attachment.ownerId) : attachment.ownerType === "lesson" ? await canAccessLesson(user, attachment.ownerId) : false;
