...
export async function createReference(formData: FormData) { const user = await actor(); const value = parse(formData); const id = await referenceRepository.createReference(inputFrom({ ...value, status: "draft" }), user.id); await saveContentAttachments(formData, "reference", id, user.id); redirect(`/admin/referencias?edit=${id}`); }
export async function updateReference(formData: FormData) { const user = await actor(); const value = parse(formData); if (!value.referenceId) throw new Error("Referência não informada."); await referenceRepository.updateReference(value.referenceId, inputFrom(value), user.id); await saveContentAttachments(formData, "reference", value.referenceId, user.id); revalidatePath("/admin/referencias"); revalidatePath("/app/biblioteca"); revalidatePath(`/app/biblioteca/${value.referenceId}`); }
