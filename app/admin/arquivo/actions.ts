...
export async function createCase(formData: FormData) { const user = await actor(); const value = parse(formData); const id = await caseRepository.createCase(inputFrom({ ...value, status: "draft" }), user.id); await saveContentAttachments(formData, "case", id, user.id); revalidatePath("/app/arquivo"); return id; }
export async function updateCase(formData: FormData) { const user = await actor(); const value = parse(formData); if (!value.caseId) throw new Error("Caso não informado."); await caseRepository.updateCase(value.caseId, inputFrom(value), user.id); await saveContentAttachments(formData, "case", value.caseId, user.id); revalidatePath("/admin/arquivo"); revalidatePath("/app/arquivo"); revalidatePath(`/app/arquivo/${value.caseId}`); }
