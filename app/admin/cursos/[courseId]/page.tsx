    ...
    <div className="admin-page content-admin-page">
          ...
          return (
            <article className="panel module-admin-card" key={module.id}>
              ...
              <footer className="module-admin-footer">
                <details className="inline-editor"><summary className="text-link"><Pencil size={14} />Editar módulo</summary><form className="cms-form compact-form" action={updateModule} encType="multipart/form-data"><input type="hidden" name="courseId" value={course.id} /><input type="hidden" name="moduleId" value={module.id} /><label>Título<input name="title" defaultValue={module.title} required /></label><label>Descrição<textarea name="description" defaultValue={module.description} rows={3} /></label><label className="checkbox-field toggle-field"><input name="isFree" type="checkbox" defaultChecked={module.isFree} /><span className="toggle-control" aria-hidden="true" /><span className="toggle-label"><span className="toggle-paid">Pago</span><span className="toggle-free">Gratuito</span></span></label><label>Status<select name="status" defaultValue={module.status}>{CONTENT_STATUSES.map((value) => <option key={value} value={value}>{statusLabel[value]}</option>)}</select></label><ContentAttachmentsField attachments={attachmentsByModule.get(module.id)} /><button className="ghost-button" type="submit">Salvar módulo</button></form></details>
      ...
      <details className="panel add-module-panel" open={modules.length === 0}>
        ...
        <form className="cms-form form-grid" action={createModule} encType="multipart/form-data">
          ...
          <label className="checkbox-field toggle-field"><input name="isFree" type="checkbox" /><span className="toggle-control" aria-hidden="true" /><span className="toggle-label"><span className="toggle-paid">Pago</span><span className="toggle-free">Gratuito</span></span></label>
