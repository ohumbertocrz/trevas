  ...
  return (
    <div className="page lesson-page">
      ...
      {lesson.vimeoEmbedUrl && isVimeoEmbedUrl(lesson.vimeoEmbedUrl) ? (
        <VimeoProgressPlayer embedUrl={lesson.vimeoEmbedUrl} lessonId={lesson.id} title={lesson.title} viewerName={user?.displayName ?? "Usuário Trevas"} viewerEmail={user?.email ?? ""} initialPositionSeconds={timestamp > 0 ? timestamp : progress?.lastPositionSeconds ?? 0} initialPercent={progress?.percent ?? 0} initialCompleted={progress?.completed ?? false} />
      ) : (
