...
export function ContentAttachmentsField({ attachments }: { attachments?: ContentAttachment[] }) {
  return <div className="full-field content-attachments-field"><span className="field-label"><Paperclip size={14} />Arquivos anexos</span><FilePicker name="attachments" multiple /><small className="form-hint">Vários arquivos, até 50 MB cada.</small>{attachments && attachments.length > 0 && <div className="content-attachment-list">{attachments.map((attachment) => <div className="content-attachment-row" key={attachment.id}><a href={`/api/content-attachments/${attachment.id}/download`}><FileText size={15} /><span>{attachment.name}</span><small>{formatAttachmentSize(attachment.sizeBytes)}</small></a></div>)}</div>}</div>;
}
