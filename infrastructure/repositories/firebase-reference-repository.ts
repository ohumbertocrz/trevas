...
function referenceFromData(id: string, data: FirebaseFirestore.DocumentData): LibraryReference {
  return { id, title: String(data.title ?? ""), author: String(data.author ?? ""), coverPath: String(data.coverPath ?? ""), description: String(data.description ?? ""), type: REFERENCE_TYPES.includes(data.type as ReferenceType) ? data.type as ReferenceType : "livro", tags: Array.isArray(data.tags) ? data.tags.filter((tag): tag is string => typeof tag === "string") : [], referenceUrl: String(data.referenceUrl ?? ""), vimeoEmbedUrl: String(data.vimeoEmbedUrl ?? ""), lessonIds: Array.isArray(data.lessonIds) ? data.lessonIds.filter((id): id is string => typeof id === "string") : [], caseIds: Array.isArray(data.caseIds) ? data.caseIds.filter((id): id is string => typeof id === "string") : [], status: CONTENT_STATUSES.includes(data.status) ? data.status : "draft", updatedAt: data.updatedAt instanceof Timestamp ? data.updatedAt.toDate() : null };
}
